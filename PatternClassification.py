...
def add_layer(layername, inputs, in_size, out_size, activation_function=None):
    ...
    with tf.variable_scope(layername, reuse=None):
        Weights = tf.get_variable("weights", shape = [in_size, out_size], initializer = tf.truncated_normal_initializer(stddev=0.1))
        biases = tf.get_variable("biases", shape = [1, out_size], initializer = tf.truncated_normal_initializer(stddev=0.1))
...
def produceData(r, w, d, num):
    ...
    Y_label2 = - np.ones(num) #类别标签为-1，注意：由于采用双曲正切函数作为激活函数，类别标签不能为0
...
def produce_random_data(r, w, d, rum):
